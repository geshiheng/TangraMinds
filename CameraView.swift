    ...
    class Coordinator: NSObject, AVCapturePhotoCaptureDelegate {
        ...
        func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
            if let error = error {
                ...
                if let databaseManager = parent.databaseManager {
                    databaseManager.insertPhoto(image: image)
                } else {
                    print("Error: DatabaseManager is not available")
                }
                sendComplete() // 添加这一行
        ...
        func sendComplete() {
            ...
            parent.bluetoothManager?.sendData(data)
        }
...
struct CameraView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let bluetoothManager = BluetoothManager() // 添加这一行
        CameraView(databaseManager: databaseManager, bluetoothManager: bluetoothManager) // 修改这一行
    }
}
